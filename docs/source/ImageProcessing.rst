...
2. Every channel with keep == 0 in the panel file will be dropped from
   ...
   the keep column after this step (unless you are re-doing this step)!
...
the steinbock pipeline. The way it works is a that a threshold number is
...
quantile value per channel per image is the threshold for hpf for pixels
in that image). This method allows the threshold to vary for channels
and images with very different intensities. A channel whose maximum
intensity is 25 will not benefit from hpf == 50, for example, whereas
sliding the threshold based on the quantile of values in the
...
be launched from the indicated button in PalmettoBUG. Launching form
...
functions & models provided in the cellpose package, or if you want a
...
try and re-create ‘clean’ images from ‘noisy’ images, they were trained
...
images of the dataset – and necessary and critical step in the analysis
of cellular imaging data. The PalmettoBUG GUI wraps two generalist cell
segmentation packages – Cellpose and DeepCell / Mesmer. These packages
offer 1 or more models that were trained on many datasets in order to
make them decent at identifying many different types of cells. These
segmentation results are frequently good enough to data analysis,
...
required, then generalist algorithms like are offered in PalmettoBUG are
...
your choosing – allowing the model to specifically good at segmenting
your cell types. Still, even if segmentation masks are generated in a
...
parameters, such as average cell size (I usually have been setting this
...
looking at only the pixels within the mask – this gets saved in one set
of .csv files (“intensities”). Meanwhile, for each mask a set of
characteristics are recorded about the shape and source of the mask is
saved (in “regionprops” csv’s). This includes things like the area and
perimeter length of each mask. The procedure / outputs for this step
...
set up within the larger image project directory, as a sub-folder in
*/Analyses*.
