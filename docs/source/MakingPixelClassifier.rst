...
To make a classifier, click on the “create, load, or overwrite” button
...
load a classifier (only supervised classifiers can be reloaded in such a
way that they can train / predict – unsupervised classifiers can be
loaded, but only for the purpose of examining their setup details).
Classifiers can be loaded from the project– any classifier made in an
imaging project will be automatically saved & available for reload
inside that project – or from PalmettoBUG itself. Supervised classifiers
can be saved to PalmettoBUG, making the classifier then available to
*any project* run in that installation of PalmettoBUG.

|image1|

Once you have chosen to create a new supervised classifier, a new window
will open where you can select the settings for it. |image2|

Key settings for this type of classifier include:
   ...
   can derive more information in additional to the plain intensity
   values. ‘Gaussian’ is included by default as it represents only the
   blurring of the image (so is analogous to intensity), while the other
   features do more radical transformations of the data. For example,
   Hessians (especially the hessian minimum) and the Laplacian use
   kernels that calculate derivatives of the image, and can help
   identify edges or areas of sharp transition in a channels’ intensity.
   The implementation of these features were directly translated from
   QuPath into python, and you can see QuPath’s documentation for more
   info about the available features and why you might use them: `Pixel
   classification — QuPath 0.5.1
   ...
   *# of data layers = (# of sigmas) \* (# of features) \* (# of
   ...
   However, if you want to find multiple classes are mutually exclusive
   (such as different cell types), then a single classifier is
   preferrable. **Note! –** images are arrays of numbers, so when the
   classifier makes pixel predictions and saves them as an image, it
   will be as numbers, and not the class labels. Use the classifier
   details button, or look at the *biological_labels.csv* in the pixel
   classifier’s folder to see how the numbers in the pixel
   classification maps correspond to the biological labels that you
   assigned.
...
when Napari is opened by the directory display), the PalmettoBUG window
will be non-responsive, **and if you try to move the PalmettoBUG window
the entire program may crash**! This error is because of how Python
handles threads. **So open any windows you may need (like the classifier
details) for labeling BEFORE launching training labels in Napari, and do
not try to move the PalmettoBUG window while Napari is open (minimizing
...
**Note! –** the set of images that the classifier will be trained on is
set by the image folder selected in the training frame – if you change
that folder of images between creating the training labels and training
the classifier, then note that the images actually used for training may
not look exactly like the images you actually the classifier on!
...
same folder that the classifier was trained on – as in, if you denoised
the images in your project and used those to train the classifier, you
probably should not predict with that classifier using the pre-denoising
images.

Next, set the prediction to do the entire folder of images or just one
image. If only predicting from one image is selected (this can be useful
to get a quick look at how the classifier appears to be performing
without needing to wait for the entire folder to be run), then use the
drop-down menu for selecting an image. This drop-down is ignored if you
choose to predict the entire folder.

Finally, run the prediction!

|image4|

**Note! –** The predictions are written to the pixel classiifer’s
/*classification_maps* sub-folder, where they are accessible by the rest
of the program for downstream steps. However, they are not automatically
displayed after prediction. You will want to examine you pixel class
predictions, to see if they are capturing the classes that you want
accurately, and to do this you should launch the classification map(s)
in Napari. This can be done in the directory display from inside
PalmettoBUG, or you can open Napari in a separate terminal from
PalmettoBUG and navigate to the relevant directories using your system
native file explorer app. If you use the directory display inside
PalmettoBUG’s pixel classification tabs, then the image will also be
loaded into Napari beneath the classification prediction, allowing you
to easily go back and forth between the predictions and the original
image.
...
in the images. The pixel values are normalized within each channel by
...
toffy), before they are introduced to the FlowSOM algorithm for
training/prediction.
...
supervised classifiers:
   ...
   sample of the pixels without needing any supervision (hence the
   names!).

   2). The output classes of an unsupervised classifier have no
   biologically-relevant label (this must be annotated by the user after
   the classifier is run). This is because, without supervision, these
   classifiers merely clump similar looking pixels together without any
   understanding of the biological label that might apply to each group
   ...
   classifiers should create much more classes than an revisedvised
   ...
   4). Some of the parameters are different. More detail on these in the
   next section.

   5). Unsupervised classifiers must be used for prediction WHEN they
   are created (they cannot be saved and re-used!). They can be saved
   and reloaded for the purposes of viewing their original training /
   setup details, but they cannot be reloaded and run again. If you need
   to duplicate a prior unsupervised classifier, you must use the first
   classifier’s details to create an identical copy to retrain and
   predict with again.
   ...
   classifiers. This abundance of small, isolated / discontinuous pixel
   class regions makes their baseline output harder to use for most
   applications *except classifying cells using a secondary FlowSOM.*
   These problematic regions can also be partly controlled by using
   “smoothing” during class predication or after prediction during the
   merging / annotation steps.
...
launch the create a classifier window. But once in this window, use the
button for creating an unsupervised classifier, which will launch a new
window. Note that whatever name you choose for the classifier will have
“Unsupervised\_” appended to it. As in, if the name you select is
...
presented with a large number of options:
   ...
   channels, except the ‘gaussian’ feature is ALWAYS used for any
   selected channel (which is why it is not visible in the list of
   features).
   ...
   **Note! –** that excessively large values for the pixel subsampling
   ...
   3). Sigma blurring & Quantile normalization. These effect the
   ...
   is normalized within each channel (default all channels are divided
   ...
   XY dimensions value squared must be greater than the number of
   metaclusters, and greater training iterations can improve the
   stability of the FlowSOM, although it takes more time to train.
   ...
   smaller than the smoothing number (fewer than that number of pixels)
   will have its values replaced by the statistical mode of the values
   of the surrounding pixels. This mode for a given pixel is calculated
   by looking first at the immediately adjacent pixels to calculate the
   mode (if all of those pixels had been part of an isolated region and
   also dropped, then the mode is calculated for an expanded area).

Running Training from this window completes the creation of the
...
**Note! –** unlike supervised pixel classifiers, **unsupervised
classifiers cannot make class predictions after being reloaded, you must
make the predictions the first time you create them** – they can be
reloaded in order to do things like examine their training parameters,
...
of the selected channels in each predicted class of the classifier, is a
key plot for helping to annotate and merge the predicted classes.
...
| Class Labels” button in the upper-left frame, which launches a window
  ...
  be ignored and dropped in the merging.

|image6|

When you launch a pixel classification map in Napari from the directory
displays in the pixel classification tabs, it will automatically open
both the pixel classification and original image in Napari. The image
will be underneath the classification layer by default, so you may need
to adjust the opacity of the class labels to see both the labels and the
...
the documentation about using a pixel classifier? Because I say so,
that’s why!
...
This style of segmentation is quite reasonable of things like nuclear
segmentation (as usually nuclei are generally circular, well separated,
etc.), where following up with an expansion of the masks can be done to
roughly encompass the cytoplasm around the nuclei.

.. |A screenshot of a computer Description automatically generated| image:: media/MakePxClass/PxMake1.png
   :width: 6.5in
   :height: 4.12778in
.. |image1| image:: media/MakePxClass/PxMake2.png
   :width: 6.5in
   :height: 2.00069in
.. |image2| image:: media/MakePxClass/PxMake3.png
   :width: 6.5in
   :height: 2.49306in
.. |Screens screenshot of a computer Description automatically generated| image:: media/MakePxClass/PxMake4.png
   :width: 5.7139in
   :height: 3.93136in
.. |image3| image:: media/MakePxClass/PxMake5.png
   :width: 6.5in
   :height: 3.26319in
.. |image4| image:: media/MakePxClass/PxMake6.png
   :width: 6.5in
   :height: 5.53611in
.. |image5| image:: media/MakePxClass/PxMake7.png
   :width: 6.53839in
   :height: 3.15813in
.. |A screenshot of a computer screen Description automatically generated| image:: media/MakePxClass/PxMake8.png
   :width: 6.5in
   :height: 4.45556in
.. |image6| image:: media/MakePxClass/PxMake9.png
   :width: 6.5in
   :height: 3.84097in
.. |image7| image:: media/MakePxClass/PxMake10.png
   :width: 6.5in
   :height: 4.32431in
