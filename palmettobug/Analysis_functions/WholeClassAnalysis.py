...
class WholeClassAnalysis:     
    ...
    def _load(self, csv: Union[pd.DataFrame, None] = None) -> None:
        ...
        if csv is not None:
            ...
            if len(fcs_dir_names) != len(self._metadata):
                ...
                fcs_dir_names = new_fcs_filenames

            fcs_path_list = ["".join([fcs_directory,"/",i]) for i in fcs_dir_names]

            intensities = pd.DataFrame()
            length_of_images = [0]
            length_of_images2 = []
            tally = 0
            for i in fcs_path_list:
    ...
    def input_tables_to_csv(self) -> None:
        '''Allows the saving of the primary csv files within this class to the disk inside the self.direcotry folder'''
    ...
    def plot_percent_areas(self, filename: Union[str, None] = None, calculate_only: bool = False) -> plt.figure:
        ...
        If filename is specified with export the plot as a PNG file to self.save_dir/{filename}.png
        '''
    ...
    def plot_distribution_exprs(self, 
        ...
        Returns the plot as a seaborn FacetGrid (FacetGrid.figure --> a matplotlib figure)
    ...
    def export_data(self, 
        ...
        Args:
            subset_columns (list[str] or None):  a list of strings denoting the columns to subset the dataframe's rows on (here and in other 
                    arguments, non-string input is attepmted to be cast to strings inside the function, as well as the corresponding 
                    column of the data). if this or subset_types is None, no subsetting occurs. 

            subset_types (list[list[str]] or None): a list contains sub-lists for strings. The length of the upper list must be the length of the
                    subset_columns list, as each sub-list contains strings corresponding to the rows to keep. 
                    ...
                    So in the above example, all rows of column1 == type2/6 that also have column2 == typeB/Z are the rows that 
                    are maintained.
                                                        
            groupby_columns (list[str] or None): a list of strings indicating what columns of the data to groupby. If None, then grouping is 
                    not performed. Used as in this example:    pandas.DataFrame.groupby(groupby_columns)

            statistic (str): possible values: 'mean','median','sum','std','count'. Denotes the pandas groupby method to be used after grouping 
                    (ignored if groupby_columns is None). Numeric methods (mean, median, sum, std) are only applied to numeric 
                    columns, so only those columns + the groupby columns will be in the final dataframe / csv

            include_marker_class_row (bool): Whether to include the marker_class information as a row at the bottom of the table

                                >>> True to include this row -- useful for reimport into PalmettoBUG. However, breaks the
                                convention on having the same datatype for every value in the columns, and 
                                would require manual editing to properly be taken up by a software other than PalmettoBUG

                                >>> False to not include this row -- this is better for import into non-PalmettoBUG software 
                                for analysis,

                    or at the least the user will need to remember to remove this row before analyzing!
                    When the marker_class row is included, it is encoded as integers (to prevent mixed dtype issues/warnings on 
                    reload) --> 0 = 'none', 1 = 'type', 2 = 'state', 3 = 'spatial_edt'
                    metadata columns (which have no marker_class) have this row filled with 'na'. 
                    NOT USED IN COMBINATION WITH GROUPING!

            filename: (str, or None): the name of the csv file to save the exported dataframe inside the self.data_table_dir folder. If None,
                    no export occurs, and the data table is only returned. 

        Returns:
            (pandas DataFrame) -- the pandas dataframe representing the exported data. 

        Inputs/Output:
            Outputs: if filename is provided, will export the final dataframe to self.data_table_dir/{filename}.csv
        '''
        if filename is None:
            output_path =  None
        else:
        ...
        if (include_marker_class_row is True) & (groupby_columns is None):
            ...
            marker_class_dict = {'none' : 0, 'type' : 1, 'state' : 2}     # so not mixed type on read
